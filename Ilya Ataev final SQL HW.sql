...
WITH tmptbl AS (SELECT artid, AVG(gain) OVER (PARTITION BY artid) as AVGGAIN, artweight FROM sales JOIN articles ON sales.artid=articles.id) SELECT DISTINCT artid, AVGGAIN / artweight as AVGCOST FROM tmptbl ORDER BY AVGCOST DESC;
